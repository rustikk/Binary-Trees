...
class BinaryTree:
    ...
    #recursion tutorial
    """
    recursion is used because when code is compiled to assembly language
    its often faster than loops are.

    DONT USE recursion if you can't guarantee an exit condition
    """
    def inorder(self):
    ...
    def find_parent(self, value: int) -> Node:
        ...
        current_node = self.head
        while current_node:
            if (current_node.left is not None and current_node.left.value == value) or\
    ...
    def find_rightmost(self, node: Node) -> Node:
        current_node = node
        #if the node to the right is not none, move to the right
